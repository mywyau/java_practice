...
public class WordSearch implements DsaExercise {
    /** Tries every cell as the beginning of a four-directional word path. */
    public static boolean exist(char[][] board, String word) {
        ...
        for (int row = 0; row < board.length; row++) {
            for (int column = 0; column < board[row].length; column++) {
                if (find(board, word, row, column, 0)) {
                    return true;
                }
            }
        }
    ...
    private static boolean find(char[][] board, String word,
        ...
        if (index == word.length()) {
            return true; // Every character has been matched.
        }
        ...
        board[row][column] = '\0'; // Mark this cell as unavailable on the current path.
        boolean found = find(board, word, row + 1, column, index + 1)
                ...
                || find(board, word, row, column - 1, index + 1);
        board[row][column] = saved; // Undo so another path can reuse the cell.
        return found;
    }
